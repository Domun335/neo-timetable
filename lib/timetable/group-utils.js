...
export function formatSubjectDisplayName(rawSubject = '') {
  ...
  if (lower.startsWith('r_ang') || lower.includes('r_angielski')) return 'Język angielski (rozszerzony)'
  if (lower.startsWith('z_ang') || lower.includes('z_angielski')) return 'Język angielski zawodowy'
  if (lower === 'j. angielski' || lower === 'j.angielski' || lower === 'angielski') return 'Język angielski'
  if (lower.startsWith('j. niem') || lower === 'j.niemiecki' || lower === 'niemiecki') return 'Język niemiecki'
      ...
      for (const lesson of slot) {
        ...
        if (lesson.groupName || lesson.groupNum) {
          ...
          if (lesson.groupNum) {
            ...
            if (!currentLabel || (newLabel.length > currentLabel.length && !currentLabel.includes('/'))) {
...
/**
 * Sprawdza czy lekcja powinna być widoczna zgodnie z preferencjami grup
 * @param {any} lesson
...
export function isLessonVisible(lesson, preferences) {
  ...
  // Obsługa starego formatu (np. liczba lub string)
  if (typeof preferences === 'number') {
    return lesson.groupNum === preferences
  }
  if (typeof preferences === 'string') {
    return lesson.groupName?.trim() === preferences.trim()
  }

  // Obsługa formatu obiektowego
  if (typeof preferences === 'object') {
    // 1. Sprawdzamy czy to nowy format z { base, subjects }
    if ('base' in preferences || 'subjects' in preferences) {
      const subjectOverride =
        preferences.subjects &&
        typeof preferences.subjects === 'object' &&
        Object.hasOwn(preferences.subjects, lesson.subject)
          ? preferences.subjects[lesson.subject]
          : undefined

      // Wyjątek per-przedmiot
      if (subjectOverride !== undefined) {
        if (subjectOverride === 'hide') return false
        if (subjectOverride === 'all' || subjectOverride === null) return true
        if (typeof subjectOverride === 'number') {
          return lesson.groupNum === subjectOverride
        }
      }

      // Brak wyjątku dla tego przedmiotu -> sprawdzamy grupę bazową
      if (preferences.base === null || preferences.base === undefined) {
        return true
      }
      if (typeof preferences.base === 'number') {
        // Inteligentny fallback: jeśli baza to np. 3, ale dany podział ma mniej grup (np. języki/WF mają max 2 grupy)
        // 1. Sprawdzamy mianownik podziału w nazwie grupy (np. "1/2", "2/2", "I/2")
        const denomMatch = (lesson.groupName || '').match(/\/([1-9]\d*)/)
        if (denomMatch) {
          const totalGroups = parseInt(denomMatch[1], 10)
          if (preferences.base > totalGroups) {
            // Grupa bazowa nie istnieje w tym podziale -> domyślnie pokazujemy lekcję
            return true
          }
        }

        // 2. Języki obce oraz WF w szkołach mają podział na max 2 grupy (np. II lub I/2)
        const cat = lesson.groupCategory
        if ((cat === 'lang' || cat === 'wf') && preferences.base > 2) {
          return true
        }

        return lesson.groupNum === preferences.base
      }
      return true
    }

    // 2. Obsługa starego formatu { general, lang, wf }
    const cat = lesson.groupCategory || 'general'
    const chosen = preferences[cat]
