...
/**
 * Tabela planu lekcji dla widoku desktopowego oraz wydruku
 * Zoptymalizowana przez React.memo — ignoruje upływ sekund/minut timera, gdy numer aktywnej lekcji się nie zmienia
 */
export const TimetableTable = memo(
            ...
            <tbody className="divide-y divide-border/60">
              {sortedHourKeys.map((hourKey, hourIndex) => {
                ...
                const isCurrentPeriod =
                  currentInfo.isSchoolDay && currentInfo.currentLessonNumber === hourObj.number
                    ...
                    <td className="p-2.5 lg:p-3 text-center align-middle border-r border-border/60 bg-muted/20 shrink-0 print:p-1.5">
                      <div className="flex flex-col items-center justify-center gap-0.5">
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <span
                            className={cn(
                              'text-sm font-semibold print:text-xs',
                              isCurrentPeriod
                                ? 'text-primary print:text-foreground'
                        ...
                        <span className="text-xs font-mono font-normal text-foreground dark:text-white whitespace-nowrap leading-tight print:text-[11px]">
                          {hourObj.timeFrom}
                        </span>
                        <span className="text-xs font-mono font-normal text-muted-foreground dark:text-[#8A909A] whitespace-nowrap leading-tight print:text-[10px]">
                          {hourObj.timeTo}
                        </span>
                      </div>
                    </td>
